...
def resolve_token(
    ...
    """Return user_token. Personal mode (phone=None) → env USER_TOKEN.
    Agent mode (phone given) → Redis → file cache → trustedBind."""
    if phone is None:
...
def main() -> None:
    ...
    if args.phone:
        if not config.admin_secret:
            die("使用 --phone（agent 模式）时 ADMIN_SECRET 必须在 .env 中配置")
    else:
        if not config.user_token:
            die("未传 --phone 时 USER_TOKEN 必须在 .env 中配置（personal 模式）；或者改用 agent 模式：传 --phone <11 位手机号> + 配置 ADMIN_SECRET")
    ...
    if not token:
        die("未能解析到 user_token，请检查 USER_TOKEN（personal 模式）或 ADMIN_SECRET（agent 模式）配置")
